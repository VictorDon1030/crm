$(function () {
    ...
    var methodObject = {
        ...
        dictionary_changeState: function () {
            //1.获取被选择的行
            var row = dictionary_datagrid.datagrid("getSelected");
            if (!row) {
                $.messager.alert("温馨提示", "请选择要停用的一行！", "warning");
                return;
            }
            var status = "";
            //2.弹出确认框：是否确认停用
            ...
            //2.弹出确认框：是否确认停用
            $.messager.confirm("温馨提示", status, function () {
                //发送ajax请求到后台，将要停用的id传过去
                $.post("/dictionary/changeState.do", {"id": row.id}, function (data) {
                    if (data.success) {
        ...
        dictionaryItem_add: function () {
            ...
            dictionaryItem_form.form("load", itemRow);
            //回显combobx:字典目录
            $("#dictionaryItem").combobox("setText", row.name);
        ...
        dictionaryItem_changeState: function () {
            //1.获取被选择的行
            var row = dictionaryItem_datagrid.datagrid("getSelected");
            if (!row) {
                $.messager.alert("温馨提示", "请选择要停用的一行！", "warning");
            ...
            $.messager.confirm("温馨提示", status, function () {
                //发送ajax请求到后台，将要停用的id传过去
                $.post("/dictionaryItem/changeState.do", {"id": row.id}, function (data) {
                    if (data.success) {
                        $.messager.alert("温馨提示", "操作成功", "info", function () {
                            //点击确定按钮,刷新datagrid
